...
#include <cmath>  // for std::isnan, std::isinf, std::abs, std::sqrt
...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    // 设置合理的默认值，避免-1导致的崩溃
    node_->declare_parameter("fsm/flight_type", 1);  // 默认手动目标点模式
    node_->declare_parameter("fsm/thresh_replan_time", 1.0);  // 默认1秒
    node_->declare_parameter("fsm/thresh_no_replan_meter", 1.0);  // 默认1米
    node_->declare_parameter("fsm/planning_horizon", 6.0);  // 默认6米
    node_->declare_parameter("fsm/planning_horizen_time", 3.0);  // 默认3秒
    ...
    // 验证target_type_参数
    if (target_type_ < 1 || target_type_ > 3) {
      RCLCPP_ERROR(node_->get_logger(), "Invalid flight_type: %d, should be 1-3. Setting to 1 (MANUAL_TARGET)", target_type_);
      target_type_ = 1;  // 默认为手动目标点模式
    }

    have_trigger_ = !flag_realworld_experiment_;

    node_->declare_parameter("fsm/waypoint_num", 1);  // 默认1个航点
    node_->get_parameter("fsm/waypoint_num", waypoint_num_);

    // 添加安全检查
    if (waypoint_num_ < 0 || waypoint_num_ > 50) {
      RCLCPP_ERROR(node_->get_logger(), "Invalid waypoint_num: %d, should be 0-50", waypoint_num_);
      waypoint_num_ = 1;  // 设置为安全默认值
    }

    RCLCPP_INFO(node_->get_logger(), "Loading %d waypoints...", waypoint_num_);

    for (int i = 0; i < waypoint_num_; i++)
    {
      // 设置合理的默认航点坐标
      double default_x = (i == 0) ? 5.0 : 0.0;  // 第一个航点在(5,0,1.5)
      double default_y = 0.0;
      double default_z = 1.5;

      node_->declare_parameter("fsm/waypoint" + to_string(i) + "_x", default_x);
      node_->declare_parameter("fsm/waypoint" + to_string(i) + "_y", default_y);
      node_->declare_parameter("fsm/waypoint" + to_string(i) + "_z", default_z);
      ...
      RCLCPP_INFO(node_->get_logger(), "Waypoint %d: (%.2f, %.2f, %.2f)",
                  i, waypoints_[i][0], waypoints_[i][1], waypoints_[i][2]);
    }

    /* initialize main modules */
    RCLCPP_INFO(node_->get_logger(), "Initializing visualization...");
    visualization_.reset(new PlanningVisualization(node_));

    RCLCPP_INFO(node_->get_logger(), "Initializing planner manager...");
    planner_manager_.reset(new EGOPlannerManager);

    RCLCPP_INFO(node_->get_logger(), "Initializing plan modules...");
    planner_manager_->initPlanModules(node_, visualization_);

    RCLCPP_INFO(node_->get_logger(), "Delivering trajectory to optimizer...");
    planner_manager_->deliverTrajToOptimizer(); // store trajectories
    planner_manager_->setDroneIdtoOpt();

    /* callback*/
    RCLCPP_INFO(node_->get_logger(), "Setting up timers and callbacks...");
    ...
    RCLCPP_INFO(node_->get_logger(), "EGO-Planner initialization completed successfully!");
    RCLCPP_INFO(node_->get_logger(), "Flight type: %d, Realworld experiment: %s",
                target_type_, flag_realworld_experiment_ ? "true" : "false");
  }
  ...
  void EGOReplanFSM::triggerCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg [[maybe_unused]])
  {
    have_trigger_ = true;
    cout << "Triggered!" << endl;
    init_pt_ = odom_pos_;
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    cout << "Received 2D Nav Goal: (" << msg->pose.position.x << ", "
         << msg->pose.position.y << ", " << msg->pose.position.z << ")" << endl;

    // 在实验模式下，2D Nav Goal应该触发规划
    if (!have_trigger_)
    {
      have_trigger_ = true;
      cout << "2D Nav Goal triggered planning!" << endl;
    }

    init_pt_ = odom_pos_;

    // 使用消息中的z坐标，但确保合理的高度
    double target_z = std::max(0.5, std::min(3.0, msg->pose.position.z));
  ...
  void EGOReplanFSM::odometryCallback(const std::shared_ptr<const nav_msgs::msg::Odometry> &msg)
  {
    ...
    if (std::isnan(x) || std::isnan(y) || std::isnan(z) ||
      ...
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                           "Invalid odometry data: NaN or Inf detected, ignoring");
      return;
    }

    // 检查位置是否在合理范围内（±150米，匹配odometry_bridge的处理范围）
    if (std::abs(x) > 150.0 || std::abs(y) > 150.0 || std::abs(z) > 150.0) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                           "Position out of reasonable range: x=%.2f, y=%.2f, z=%.2f, ignoring", x, y, z);
      return;
    }

    // 检查速度是否在合理范围内（±50 m/s）
    if (std::abs(vx) > 50.0 || std::abs(vy) > 50.0 || std::abs(vz) > 50.0) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                           "Velocity out of reasonable range: vx=%.2f, vy=%.2f, vz=%.2f, ignoring", vx, vy, vz);
      return;
    }
    ...
    if (std::isnan(quat_norm) || quat_norm < 0.9 || quat_norm > 1.1) {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                           "Invalid quaternion: norm=%.3f, ignoring", quat_norm);
      return;
    }
    ...
    // 归一化四元数
    odom_orient_.w() = qw / quat_norm;
    odom_orient_.x() = qx / quat_norm;
    odom_orient_.y() = qy / quat_norm;
    odom_orient_.z() = qz / quat_norm;

    have_odom_ = true;

    // 调试信息：定期输出有效的里程计数据
    RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 5000,
                         "Valid odometry: pos(%.2f, %.2f, %.2f), vel(%.2f, %.2f, %.2f)",
                         x, y, z, vx, vy, vz);
  }
