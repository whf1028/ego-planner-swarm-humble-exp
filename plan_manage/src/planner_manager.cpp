...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(rclcpp::Node::SharedPtr &node, PlanningVisualization::Ptr vis)
  {
    ...
    // A*节点池动态配置参数 - 恢复合理大小
    node->declare_parameter("manager/astar_pool_x", 80);
    node->declare_parameter("manager/astar_pool_y", 80);
    node->declare_parameter("manager/astar_pool_z", 30);
    ...
    // 获取A*节点池配置
    int astar_pool_x, astar_pool_y, astar_pool_z;
    node->get_parameter("manager/astar_pool_x", astar_pool_x);
    node->get_parameter("manager/astar_pool_y", astar_pool_y);
    node->get_parameter("manager/astar_pool_z", astar_pool_z);
    ...
    // 动态A*节点池配置：根据飞行环境自适应调整
    Eigen::Vector3i astar_pool_size(astar_pool_x, astar_pool_y, astar_pool_z);
    bspline_optimizer_->a_star_->initGridMap(grid_map_, astar_pool_size);

    // 输出配置信息
    RCLCPP_INFO(node->get_logger(), "A*节点池配置: %dx%dx%d (%d个节点)",
                astar_pool_x, astar_pool_y, astar_pool_z,
                astar_pool_x * astar_pool_y * astar_pool_z);
    double search_range_x = astar_pool_x * 0.1;  // step_size = 0.1m
    double search_range_y = astar_pool_y * 0.1;
    double search_range_z = astar_pool_z * 0.1;
    RCLCPP_INFO(node->get_logger(), "搜索范围: %.1fm×%.1fm×%.1fm",
                search_range_x, search_range_y, search_range_z);
