...
// #include "quadrotor_msgs/msg/position_command_px4ctrl.hpp"
...
void cmdCallback()
{
  ...
  // position
  // cmd.pos.x = static_cast<float>(pos(0));
  // cmd.pos.y = static_cast<float>(pos(1));
  // cmd.pos.z = static_cast<float>(pos(2));
  cmd.position.x = pos(0);
  cmd.position.y = pos(1);
  cmd.position.z = pos(2);

  // velocity
  // cmd.vel.x = static_cast<float>(vel(0));
  // cmd.vel.y = static_cast<float>(vel(1));
  // cmd.vel.z = static_cast<float>(vel(2));
  cmd.velocity.x = vel(0);
  cmd.velocity.y = vel(1);
  cmd.velocity.z = vel(2);

  // acceleration
  // cmd.acc.x = static_cast<float>(acc(0));
  // cmd.acc.y = static_cast<float>(acc(1));
  // cmd.acc.z = static_cast<float>(acc(2));
  cmd.acceleration.x = acc(0);
  cmd.acceleration.y = acc(1);
  cmd.acceleration.z = acc(2);

  // jerk (not available here) -> set zero
  // cmd.jerk.x = 0.0f;
  // cmd.jerk.y = 0.0f;
  // cmd.jerk.z = 0.0f;

  // heading vector (unused) -> set zero
  // cmd.heading[0] = 0.0f;
  // cmd.heading[1] = 0.0f;
  // cmd.heading[2] = 0.0f;
